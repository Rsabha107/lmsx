...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!Schema::hasColumn('team_stays', 'team_id')) {
            ...
            // Migrate data
            DB::statement('
                UPDATE team_stays ts
                INNER JOIN teams t ON ts.team_code = t.code
                SET ts.team_id = t.id
            ');
        }

        // Drop constraints if team_code exists
        if (Schema::hasColumn('team_stays', 'team_code')) {
            $foreignKeys = DB::select("
                SELECT CONSTRAINT_NAME 
                FROM information_schema.KEY_COLUMN_USAGE 
                WHERE TABLE_SCHEMA = DATABASE() 
                AND TABLE_NAME = 'team_stays' 
            ...
            foreach ($foreignKeys as $fk) {
                try {
                    DB::statement("ALTER TABLE team_stays DROP FOREIGN KEY {$fk->CONSTRAINT_NAME}");
                } catch (\Exception $e) {
            ...
            foreach ($indexes as $idx) {
                try {
                    DB::statement("ALTER TABLE team_stays DROP INDEX {$idx->INDEX_NAME}");
                } catch (\Exception $e) {
                    // Ignore if already dropped
                }
            }
        ...
        Schema::table('team_stays', function (Blueprint $table) {
            ...
            if (!DB::selectOne("SHOW COLUMNS FROM team_stays WHERE Field = 'team_id' AND `Null` = 'NO'")) {
            ...
            if (!DB::selectOne("
                ...
                WHERE TABLE_SCHEMA = DATABASE() 
                AND TABLE_NAME = 'team_stays' 
                AND INDEX_NAME = 'team_stays_event_id_team_id_index'
            ")) {
        ...
        if (Schema::hasColumn('team_stays', 'team_code')) {
            try {
                DB::statement("ALTER TABLE team_stays DROP COLUMN team_code");
            } catch (\Exception $e) {
    ...
    public function down(): void
    {
        Schema::table('team_stays', function (Blueprint $table) {
            ...
            $table->dropIndex(['event_id', 'team_id']);
